...
public class SplineInterpolator : MonoBehaviour
{
	...
	float GetNormalizedTime(int idxPrev, float t, int idxNext)
	{
		...
		Debug.LogWarning("idxPrev: " + idxPrev + ", idxNext: " + idxNext);
		Debug.LogWarning(mNodes.Count);


		float ret = (t - mNodes[idxPrev].GetLeaveTime()) / 
						(mNodes[idxNext].ArrivalTime - mNodes[idxPrev].GetLeaveTime());

		// TODO: this thing is broken and returns NaN here although it should not
		// I "fixed" this byreturning 0 instead, which is sure to break other stuff 
		Debug.Log("ret: " + ret);				
		Debug.Log(float.IsNaN(ret));		
	...
	Vector3 GetHermiteInternal(int idxFirstPoint, float t)
	{
		...
		else if (idxFirstPoint > 0 && idxFirstPoint < mNodes.Count - 2)
		{
			...
			float tension = 1.1f;	// 0.5 equivale a catmull-rom
